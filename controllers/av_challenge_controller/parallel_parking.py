...
class RRT:
    def __init__(self):
        ...
        self.time_step = 0.01
        # the time to accelerate from 0 to 100 meters/sec in seconds
        self.zeroToOneHundred = 10 * 1000. / 3600.
    ...
    def sample_random_point(self):
        sampled_pos = [np.random.uniform(dim[0], dim[1]) for dim in self.state_bounds]
    ...
    def state_is_valid(self, state):
        # TODO: check bounding box for collision
        ...
        for obs in self.obstacles:
            x_min = obs.position[0] - obs.x_length/2.0
            x_max = obs.position[0] + obs.x_length/2.0
            y_min = obs.position[1] - obs.y_length/2.0
            y_max = obs.position[1] + obs.y_length/2.0
    ...
    def goal_reached(self, state):
        ...
        if abs(state[0] - self.goal_state[0]) > 1:
            return False
        # within one meter on both sides in y direction
    ...
    def steer(self, current_node, target_speed, steering_angle):
        ...
        current_speed = current_node.final_speed
        path = [state]
    ...
    def generate_graph(self, max_nodes=500):
        ...
        while len(node_list) < max_nodes and not goal_found:
            ...
            # print("nearest neighbor: " + str(neighbor.point) + "; sampled point: " + str(sampled_point))
            ...
            for speed in np.linspace(-30, 30, 13):
                for steering_angle in np.linspace(-0.7, 0.7, 15):
                    ...
                    if dist < closest_distance:
                        ...
                        best_action = (speed, steering_angle, final_speed, path)

            if best_action is not None:
                speed, steering_angle, final_speed, path = best_action
                sampled_node = Node(path[-1], neighbor)
                sampled_node.speed = speed
                sampled_node.steering = steering_angle
                sampled_node.final_speed = final_speed
        ...
        best_node, dist = self.get_nearest_node(node_list, self.goal_state)
    ...
    def visualize_2D_graph(self, node_list):
        fig = plt.figure()
    ...
    def visualize_3D_graph(self, node_list):
        fig = plt.figure()
        ax = Axes3D(fig)
...
def main():
    ...
    print(actions)
    rrt.visualize_3D_graph(node_list)
